...
defmodule Commit do
  ...
  defp make_task_title(commit_title) when commit_title != nil do
    wrap Tokenizer.split(commit_title), handle: fn tokens ->
      [ head | tail ] = tokens
      case head[:word] |> lemmatize do
        {:ok, lemma} ->
  ...
  def parse(title, description \\ nil) when title != nil do
    ...
    # IO.inspect parsed_title
    # IO.inspect description_props
